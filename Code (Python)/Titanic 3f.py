# -*- coding: utf-8 -*-
"""
Vou inserir mais features (Sex, Pclass, Fare)
"""
...
x_test_feature = pd.get_dummies(test_na[x_feature])
...
print("Melhor Pontuação (Score) Random Search: %.2f" % melhor_pontuacao_rs) #0,82
print("Melhor Pontuação (Score) Grid Search: %.2f" % melhor_pontuacao_gs) #0,82


#%% Output do modelo

output_RF_GS_3f = pd.DataFrame({'PassengerId': test_na.PassengerId, 'Survived': y_prediction_target_gs})
output_RF_GS_3f.to_csv('G:/O meu disco/Formação/Kaggle/Kaggle Titanic/Outputs/RF_GS (3f).csv', index=False)
